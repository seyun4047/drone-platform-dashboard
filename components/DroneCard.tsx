...
const DroneCard: React.FC<DroneCardProps> = ({
  ...
  return (
    <div
      ...
      className={`glass group relative overflow-hidden transition-all duration-300 rounded-xl p-4 flex flex-col min-[1201px]:flex-row gap-6 items-center cursor-pointer border ${
        isSelected
    ...
    >
      ...
      <div className="w-full min-[1201px]:w-48 flex flex-col">
        <div className="flex items-center gap-2 mb-1">
          <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse shadow-[0_0_8px_rgba(34,197,94,1)]"></div>
          <h3 className="font-futuristic text-lg text-white glow-text-blue uppercase tracking-tighter">
            {drone.name}
          </h3>
        </div>
        <div className="text-[8px] text-sky-500/40 font-mono uppercase tracking-[0.2em]">
          Connection: Stable
        </div>
      </div>
      ...
      <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-4 w-full opacity-80 group-hover:opacity-100 transition-opacity">
        <div className="flex flex-col gap-1">
          ...
          <div
            className={`text-xl font-bold font-mono ${(telemetry?.power || 0) < 20 && telemetry?.power !== null ? "text-red-400" : "text-sky-100"}`}
          >
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="text-xl font-bold text-sky-100 font-mono">
            {renderValue(telemetry?.speed, "km/h")}
          </div>
        </div>
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2 text-sky-500/70 text-[10px] font-futuristic uppercase">
          ...
          <div className="text-[10px] font-bold text-sky-300 font-mono leading-tight">
            LAT: {telemetry?.latitude?.toFixed(4) ?? "N/A"}
            <br />
      ...
      {/* Event Preview Feed */}
      <div className="w-full min-[1201px]:w-48 shrink-0 relative group/img">
        <div className="aspect-video relative overflow-hidden rounded-lg border border-sky-500/20 bg-black/40">
          {event?.event_detail?.image ? (
            ...
            <div className="w-full h-full flex flex-col items-center justify-center text-red-500/40 bg-[radial-gradient(circle_at_center,_rgba(239,68,68,0.1)_0%,_transparent_70%)] relative">
              ...
              <span className="text-[10px] mt-2 font-futuristic tracking-[0.2em]">
                NO SIGNAL
              </span>
            </div>
          )}
